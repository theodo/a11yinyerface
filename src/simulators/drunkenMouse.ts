...
const SHAKE_FREQUENCY_MS = 250;
let shakeIntervale: NodeJS.Timer;

export const activeDrunkenMouse = () => {
  scriptImportedFromExternalRepository();
};

export const removeDrunkenMouse = () => {
  // TODO: Remove the styles from the cursor
  clearInterval(shakeIntervale);
};
...
const scriptImportedFromExternalRepository = () => {
  ...
  function setOffset() {
    offsetX = random(-SHAKE_SPEED, SHAKE_SPEED);
    offsetY = random(-SHAKE_SPEED, SHAKE_SPEED);
  }
  ...
  shakeIntervale = setInterval(setOffset, SHAKE_FREQUENCY_MS);
};
